---
name: story-1.3-enemy-hp-system
overview: 完善敌人血量系统：EnemyTankBase 添加 max_hp/current_hp 属性，take_damage() 实现实际扣血和死亡逻辑
todos:
  - id: add-hp-attributes
    content: 在 EnemyTankBase.gd 添加 max_hp 和 current_hp 属性
    status: completed
  - id: init-hp-in-ready
    content: 在 EnemyTankBase._ready() 中初始化 current_hp = max_hp
    status: completed
    dependencies:
      - add-hp-attributes
  - id: complete-take-damage
    content: 完善 EnemyTankBase.take_damage() 实现血量扣减和死亡判定
    status: completed
    dependencies:
      - add-hp-attributes
  - id: update-basic-enemy
    content: 确认 BasicEnemyTank 默认 max_hp = 1（继承自基类）
    status: completed
  - id: update-docs
    content: 更新 sprint1_dev_plan.md 和 AI2AI.md 标记完成
    status: completed
    dependencies:
      - add-hp-attributes
      - init-hp-in-ready
      - complete-take-damage
---
